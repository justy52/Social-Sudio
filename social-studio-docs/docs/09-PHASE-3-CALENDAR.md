...
- Today queue separates due items into `To Post Today` and `Posted Today`
- Posted cards have a subtle completed treatment with a check badge and visible timestamp
- Undo posted restores scheduled items to Today/Upcoming when `scheduledAt` exists
- Undo posted on exported unscheduled items clears completion while keeping exported status
...
- [x] Today queue separates `To Post Today` and `Posted Today`
- [x] Posted Today cards show completed styling
- [x] Undo posted restores scheduled-for-today items to `To Post Today`
- [x] Undo posted restores future scheduled items to `Upcoming`
- [x] Undo posted keeps exported unscheduled items in `Exported`
- [x] Undo posted regression was tested and fixed
...
- [x] Phase 3.1, 3.2, 3.3, manual posted completion, and Today Queue polish work in Vercel Preview
